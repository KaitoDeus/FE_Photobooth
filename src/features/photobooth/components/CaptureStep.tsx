...
const CaptureStep: React.FC<CaptureStepProps> = ({
  ...
  return (
    <div className="relative w-full h-full min-h-[600px] flex flex-col items-center justify-center p-6">
      ...
      <div className={`relative overflow-hidden w-auto shadow-2xl rounded-2xl bg-black ${getPreviewStyle()}`}>
          <video 
            ref={videoRef} 
            autoPlay 
            playsInline 
          ...
          <div className={`absolute inset-0 bg-white transition-opacity duration-150 pointer-events-none z-50 ${flash ? 'opacity-100' : 'opacity-0'}`} />

          {countDown && (
            <div className="absolute inset-0 flex items-center justify-center z-10 bg-black/20 backdrop-blur-[2px]">
              <span className="text-9xl font-bold text-white drop-shadow-lg animate-ping font-mono">{countDown}</span>
            </div>
          )}
      </div>
